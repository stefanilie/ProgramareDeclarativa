...
let rec reduce = function 
  ...
  | (AtribWhen(Loc(l,loc'),v,e3,loc),s) when is_val v ->
    (match reduce(e3,s) with
      | Some(e3',s') -> Some(AtribWhen(Loc(l,loc'),v,e3',loc),s')
      | None -> None
    )
  | (AtribWhen(Loc(l,loc'),e2,e3,loc),s) ->
    (match reduce(e2,s) with
      | Some(e2',s') -> Some(AtribWhen(Loc(l,loc'),e2',e3,loc),s')
      | None -> None
    )
  | (AtribWhen(e1,e2,e3,loc),s) ->
    (match reduce(e1,s) with
      | Some(e1',s') -> Some(AtribWhen(e1',e2,e3,loc),s')
      | None -> None
    )
