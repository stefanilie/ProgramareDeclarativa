...
type expr =                                    (** e ::= *)
  ...
  | AtribWhen of expr * expr * expr * locatie      (** | e := e when e*)
  | Loc of int * locatie                           (** | l  {i as a value} *)
  | ReadInt of locatie
  | PrintInt of expr * locatie
...
let exps : expr -> expr list  = function
 ...
 | Var _ | Skip _ | ReadInt _ 
   -> []
 | Ref (e,_) | Deref (e,_) | Fun(_,_,e,_) | PrintInt (e,_)
   -> [e]
 | Atrib(e1,e2,_) | Op(e1,_,e2,_) | Secv(e1,e2,_) | While(e1,e2,_) 
 | App(e1,e2,_) | Let (_,e1,e2,_) | LetRec (_,_,e1,e2,_) 
   -> [e1;e2]
 | If(e1,e2,e3,_) | AtribWhen(e1,e2,e3,_)
   -> [e1;e2;e3]
 | For(e1,e2,e3,e4,_)
   -> [e1;e2;e3;e4]
...
let revExps : expr * expr list -> expr = function
   ...
   | (AtribWhen(_,_,_,loc),[e1;e2;e3]) -> AtribWhen(e1,e2,e3,loc)
   ...
   | (PrintInt(_,loc), [e]) -> PrintInt(e,loc)
...
let string_of_expr e = 
  ...
  (*| (PrintInt (i,_),_) -> "print_int " ^ string_of_int i*)
  | (ReadInt _,_) -> "read_int()"
  ...
  | (Atrib _,[s1;s2]) -> "(" ^ s1 ^ " := " ^ s2 ^ ")"
  | (AtribWhen _,[s1;s2;s3]) -> "(" ^ s1 ^ " := " ^ s2 ^ " when " ^ s3 ^ ")"
  | (If _,[s1;s2;s3]) ->
    "(if " ^ s1 ^ "\nthen " ^ s2 ^ "\nelse " ^ s3 ^ ")"
  | (While _,[s1;s2]) ->
    "while " ^ s1 ^ " do \n" ^ s2 ^ "\ndone"
