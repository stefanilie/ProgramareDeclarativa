...
let rec infertype (m:(string*tip) list) : expr -> tip = function
  ...
  | Boxed(e1,_) -> TBox (infertype m e1)
  | Unbox _ -> TUnbox
  | Fst _ -> TFst
  | Snd _ -> TSnd
  | Var (v,loc) -> (try 
  ...
  | App (e1, e2, _) -> (match (infertype m e1, infertype m e2) with
     | (TUnbox, TBox(t1)) -> t1     (*Unbox Scoate prima expresie*)     
     | (TFst, TPair(t1, t2)) -> t1  (* Pair socate prima expresie*)  
	   | (TSnd, TPair(t1, t2)) -> t2  (* Pair scoate a doua expresie*)
  ...
  | Pair (e1, e2, _)
	-> TPair (infertype m e1, infertype m e2)
...
let type_check e = try
     let t = infertype [] e in let _ =  Printf.eprintf "The type of the whole expression is  %s" (string_of_tip t) in true
  with 
