...
let rec infertype (m:(string*tip) list) : expr -> tip = function
  ...
  | Boxed(e1,_) -> TBox (infertype m e1)  (*ADAUGAT*)
  | Unbox _ -> TUnbox   (*ADAUGAT*)
  | Fst _ -> TFst   (*ADAUGAT*)
  | Snd _ -> TSnd   (*ADAUGAT*)
  | Var (v,loc) -> (try 
           lookup v m 
  ...
  | App (e1, e2, _) -> (match (infertype m e1, infertype m e2) with
     | (TUnbox, TBox(t1)) -> t1     (*Unbox Scoate prima expresie*)    (*ADAUGAT*)    
     | (TFst, TPair(t1, t2)) -> t1  (* Pair socate prima expresie*)    (*ADAUGAT*)
     | (TSnd, TPair(t1, t2)) -> t2  (* Pair scoate a doua expresie*)   (*ADAUGAT*)
