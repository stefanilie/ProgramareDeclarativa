...
let rec reduce = function
  ...
  | (Op(e,Pct,e1,loc),s)                                       (*Evaluam expresia operatorului punct*)  (*ADAUGAT*)
     -> (match reduce (e,s) with                                           (**la fel ca in readme ai la operatorul punc, se va face stg jos se duce cu match reduce in stg sus, care face some de dr sus si se duce in dr jos*)
	     |Some(e',s') -> Some (Op(e',Pct,e1,loc),s')
	     |None -> None)
  | (Op(Rec (x,e1,y,e2,_),Pct,x,loc),s) -> Some (e1,s)         (*Returnam valoarea etichetei x*)   (*ADAUGAT*) (** ultimul rand din readme cu  una pt x una pt y*) 
 ...
 (*Struct*)
  | (Rec (x, e1, y, e2, loc), s)                               (*Evaluam e1 pana la o valoare atunci cand e2 este o expresie*)   (*ADAUGAT*)
     -> (match reduce (e1,s) with 						(**cand acel e1' va fi valuare se va opri si nu va mai cicla.altfel va face some rec x e1' pana in e1 va fi valuare.*)
