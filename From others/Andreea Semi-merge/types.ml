...
let rec infertype (m:(string*tip) list) : expr -> tip = function
  ...
  | Rec (x,e1,y,e2,_)                                                  (*ADAUGAT*)
    -> (match (infertype m x, infertype m y) with
	   | (t1,t2) -> TRec(x,t1,y,t2))  
  |Foldleft(e1,e2,e3,_)                                                (**here**)
     -> (match (infetype m e1, infertype m e2,infertype m e3) with
            |(t1,t2,t3) -> TFold(TArrow(t1,TArrow(t2,t3)),t1,(t2,t1)) 
                                                                      (**PAIRS**)			
