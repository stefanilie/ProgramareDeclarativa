...
type tip = TInt 
  ...
  | TRec of string * tip * string * tip  (*ADAUGAT*)
  | TFold of tip * tip * tip              (**here**)
...
let rec string_of_tip = function
  ...
  | TFold (t1,t2,t3) ->  "(" ^string_of_tip t1 ^ "," ^ string_of_tip t2 ^ "," ^ string_of_tip t3 ^ ")"    (**here**)
...
type expr =                                    (** e ::= *)
  ...
  | Rec of string * expr * string * expr * locatie (** | {x = e1; y = e2;}*)  (*ADAUGAT*)
  |Foldleft of expr * expr * expr * locatie        (** foldleft(expr,expr,expr)**) (**here**)
  (*Pairs*)
  | Pair of expr * expr * locatie                  (** Pair of expressions -> (e1,e2)*)
  | Fst of expr * locatie                          (** | first of pair*)
  | Snd of expr * locatie                          (** | second or pair*)
...
let location = function
  ...
  | Let (_,_,_,l) | LetRec (_,_,_,_,l) | Rec(_,_,_,_,l)                             (*ADAUGAT*)
  |Foldleft(_,_,_,l)                                                                (**here**)
   | InjL (_,_,l) | InjR (_,_,l) | Match (_,_,_,l)                                   (*pairs*)
  -> l
...
let exps : expr -> expr list  = function
 ...
 | App(e1,e2,_) | Let (_,e1,e2,_) | LetRec (_,_,e1,e2,_) | Rec(_,e1,_,e2,_)             (*ADAUGAT*)
   -> [e1;e2]
 |Foldleft(e1,e2,e3,_)(**here**)
 | If(e1,e2,e3,_)
   -> [e1;e2;e3]
  
 | For(e1,e2,e3,e4,_)    |Match(e1,e2,e3,_)                                            (*pairs*)        
   -> [e1;e2;e3;e4]
...
let revExps : expr * expr list -> expr = function
   ...
   | (Rec(x,_,y,_,loc),[e1;e2]) -> Rec(x,e1,y,e2,loc)                          (*ADAUGAT*)
   | (Foldleft(_,_,_,loc),[e1,e2,e3,loc])-> Foldleft(e1,e2,e3,loc)             (**here**)
                                                                               (**PAIRS**)
...
let string_of_expr e = 
  ...
  | (App _, [s1;s2]) -> 
    " (" ^ s1 ^ s2 ^ ")"
  | (Rec (x,_,y,_,_),[s1,s2]) -> "{" ^ x ^ "=" ^ s1 ^ ";" ^ y ^ "=" ^ s2 ^ ";" ^ "}"   (**Sintaxa record*)  (*ADAUGAT*)
  | (Foldleft(_,_,_,_),[s1,s2,s3]) -> "foldleft ("^s1^","^s2^","^s3^")"              (**here**)
                                                                                     (**PAIRS**)
