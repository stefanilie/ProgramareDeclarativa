...
let rec reduce = function
  ...
  | (App (Fun(x,_,e1,_),e2,_),s) when is_val e2 -> Some (subst x e2 e1, s) (*evaluare stricta*)
  ...
  | (App (e1, e2, loc), s) ->
      (match reduce (e1,s) 
       with Some (e1',s') -> Some (App(e1',e2,loc),s')
          | None -> None)
 (* | (App (Fun(x,_,e1,_),e2,_),s) -> Some (subst x e1 e2, s) (*evaluarea non-stricta---- l|->5*) *)
  | (Let (x,_,e1,e2,_),s) when is_val e1 -> Some (subst x e1 e2, s)
  | (Let (x,t,e1,e2,loc),s) -> ( match reduce (e1,s) 
                               with Some (e1',s') -> Some (Let(x,t,e1',e2,loc), s')
							  | None -> None)
  | _ -> None                                                    (*default*)
