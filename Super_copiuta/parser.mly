...
%token LPAREN RPAREN
%token LAC RAC  /* acolade */  						/*ADAUGAT*/
%token FUN COLON
%token LET REC IN
%token REF
%token TINT TBOOL TUNIT TFLOAT

%token MATCH WITH INJL INJR PIPE            /*PAIRS*/
%token FST
%token SND
...
expr:
  ...
  | expr PCT expr				{Op ($1, Pct, $3, location())}      /*Adaugat*/
  ...
  */
  ...
  | LET VAR EQ expr IN expr %prec FUNX
                               { Let ($2, $4, $6, location()) }
  | LAC VAR EQ expr SEQ VAR EQ expr SEQ RAC  {Rec ($2, $4, $6, $8, location())}   /*record*/  			/*ADAUGAT efectiv cum e sintaxa*/ 
  | LPAREN expr CMM expr CMM expr RPAREN (FLFT($2, $4, $6, location()))           /*foldleft*/            /*ADAUGAT efectiv cum e sintaxa*/
...
funexpr:
  ...
  | LPAREN expr COM expr RPAREN { Pair($2,$4,location()) }   /*Adaugat*/  
;
